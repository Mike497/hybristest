 ...
 *  
 */
package org.astore.cockpits.setup;
...
import org.astore.cockpits.constants.AstoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = AstoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(AstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(AstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						AstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/astorecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
